...
print("="*80)
print("rocket ignition - optimized cpa attack")
print("="*80)
...
print(f"shape: {traces_array.shape}")
...
for byte_idx in range(12):
    ...
    char = chr(best_guess) if 32 <= best_guess < 127 else '?'
    print(f"byte {byte_idx}: {best_guess:3d} (0x{best_guess:02x}, '{char}') corr={best_corr:.4f}")
